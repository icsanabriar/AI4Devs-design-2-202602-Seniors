...
| **US-006** | As a **recruiter**, I want to share an assessment link with a candidate and manually record the outcome so that I can track assessment results within the ATS without needing a vendor API. | Link-out URL is configurable per job; recruiter can mark pass / fail / pending; outcome appears on candidate profile. | FR-015, FR-016, FR-017 |
...
All four previously open technical questions have been resolved as architectural decisions below.
...
- **Link-out assessment:** An assessment flow where the candidate is redirected to an external URL; the ATS records the outcome manually — no vendor API integration.
