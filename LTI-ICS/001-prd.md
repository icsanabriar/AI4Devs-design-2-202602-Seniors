...
The recommended direction is a **single coherent ATS workspace** with shared pipeline visibility, configurable stages aligned to the seven lifecycle areas, integrations to the **company careers page**, **Google Calendar**, and **Microsoft Outlook/Teams**, and an **audit-friendly** activity history for screening and decisions. **Online assessments** in MVP use **signed, expiring per-application link-out** URLs and **manual** outcome capture in the ATS (**FR-016**), not third-party assessment **APIs**. **External job board** integrations are also deferred to post-MVP phases. MVP should prove collaboration + automation value on one primary persona pair (recruiter + hiring manager) before expanding depth in compliance, global payroll, or niche HRIS variants.
...
| Stakeholder | Type | Role in the product |
|-------------|------|---------------------|
| **Recruiter / TA** | Internal user | Primary operator of the hiring pipeline |
| **Hiring Manager** | Internal user | Reviews candidates, provides feedback, approves hires |
| **TA / System Admin** | Internal user | Configures the platform, access control, and integrations |
| **Candidate** | External user | Applies to open roles; checks application status |
| **Customer Success (LTI)** | LTI internal | Supports pilot customers through onboarding and UAT |
| **Pilot customers** | External customer | First adopters validating the MVP; sign acceptance |
| **Legal / Compliance (LTI)** | LTI internal | Validates LGPD and Ley 1581 (Colombia) compliance for LATAM operations |
...
- **Integration adapters** for the careers page, calendar (Google + Outlook), and email at MVP breadth. Assessment vendor API integration is deferred to Next phase; MVP uses **signed, expiring per-application link-out** URLs with **manual** outcome recording in-app (**FR-016**; detail in **## Architectural decisions (resolved open questions)**).
...
| **Candidate** | Person *(tenant view)* | `id`, `organization_id`, `name`, `contact` | belongs to **Organization**; *submits* **Application** *assumption (MVP): candidate profile is **organization-scoped**; dedupe strategy resolved: **email as primary key**, manual merge by admin — see **Decisions log**; cross-org global identity is out of scope for MVP* |
